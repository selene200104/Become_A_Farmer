...
public class Field_1 extends JFrame {
	...
	Field_1() {
		...
		informationPanel.setBounds(0, 0, 350, 200);
		informationPanel.setBackground(Color.pink);
		container.add(informationPanel);
		
		selectPanel.setLayout(null);
		selectPanel.setBounds(0, 200, 350, 150);
		selectPanel.setBackground(Color.yellow);
		container.add(selectPanel);
		
		/* 이미지를 넣을 라벨과 아이콘 설정 
		ImageIcon statusImage = new ImageIcon("../images/felid_150x150.jpg");
		System.out.println("statusImage"+statusImage);
		JLabel imageLabel = new JLabel(statusImage);
		
		//imageLabel.setText(" 레이블 글자 ");
		imageLabel.setBounds(20, 20, 150, 150);
		...
		*/
		...
		crops.setOpaque(true);fertilizer.setOpaque(true);
            	...
            	if(seedsSelectCount == 0) {
            		...
            		if(Lettuce.seedsAmount > 0) {
                		...
                		FarmGame.hp = FarmGame.hp-3;
                   		...
                   		//상추 성장 스레드 시작
                   		//LettuceGrowthOfFiled1 lettuce = new LettuceGrowthOfFiled1();
                       	//lettuce.start();
                        	
                       	/*if(Filed2_carrot.carrotCount != 0) {
                       		filedValue++;	
            ...
            public void actionPerformed(ActionEvent ae){		
            	...
            	if(seedsSelectCount == 0) {
            		...
            		if(Carrot.seedsAmount > 0) {
                		...
                		//당근 성장 스레드 시작
                		CarrotGrowthOfField1 carrotGrow = new CarrotGrowthOfField1();
                		carrotGrow.start();
            ...
            public void actionPerformed(ActionEvent ae){
        		...
        		if(seedsSelectCount == 0) {
        			...
        			if(Radish.seedsAmount > 0) {
        				...
        				//무 성장 스레드 시작
        				RadishGrowthOfField1 radishGrow = new RadishGrowthOfField1();
        				radishGrow.start();
       	...
       	harvest.addActionListener (new ActionListener(){
            public void actionPerformed(ActionEvent ae){
    			...
    			if(fieldState == 2) {
    				...
    				if(LettuceGrowthOfField1.lettuceAmount> 0) {
    					
    					//원래있던 상추 갯수에 수확한 상추 갯수를 추가하고 수확한 값을 0으로 초기화 시킴
            			Lettuce.amount += LettuceGrowthOfField1.lettuceAmount;
            			LettuceGrowthOfField1.lettuceAmount -= LettuceGrowthOfField1.lettuceAmount;
            			System.out.println(LettuceGrowthOfField1.lettuceAmount);
            			...
            			//System.out.println("삭제후 수확 가능한 상추 개수"+LettuceGrowthOfFiled1.lettuceAmount);
    				...
    				if(CarrotGrowthOfField1.carrotAmount> 0) {
                    	
    					//원래 있던 당근 갯수에 수확한 당근 갯수를 추가하고 수확한 값을 0으로 초기화 시킴
            			Carrot.amount += CarrotGrowthOfField1.carrotAmount;
            			CarrotGrowthOfField1.carrotAmount -= CarrotGrowthOfField1.carrotAmount;
            			System.out.println(CarrotGrowthOfField1.carrotAmount);
            			...
            			//System.out.println("삭제후  씨앗 선택 값"+seedsSelectCount);
            			//System.out.println("삭제후 수확 가능한 당근 개수"+CarrotGrowthOfFiled1.carrotAmount);
            			//System.out.println("삭제후 필드 값"+fieldState);
    				...
    				//수확가능한 무 개수가 0개 이상일때
    				if(RadishGrowthOfField1.radishAmount> 0) {
                    	
            			//원래 있던 무 갯수에 수확한 무 갯수를 추가하고 수확한 값을 0으로 초기화 시킴
            			Radish.amount += RadishGrowthOfField1.radishAmount;
            			RadishGrowthOfField1.radishAmount -= RadishGrowthOfField1.radishAmount;
            			System.out.println(RadishGrowthOfField1.radishAmount);
            			...
            			//System.out.println("삭제후  씨앗 선택 값"+seedsSelectCount);
            			//System.out.println("삭제후 수확 가능한 무 개수"+RadishGrowthOfFiled1.radishAmount);
            			//System.out.println("삭제후 필드 값"+fieldState);
